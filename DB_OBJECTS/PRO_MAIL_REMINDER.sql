create or replace PROCEDURE PRO_MAIL_REMINDER(pComp_Aid VARCHAR2,pEmp_Aid VARCHAR2 )
/*CREATE BY Mr SAROJ KUMAR GIRI*/
IS
...
BEGIN
OPEN curRec FOR SELECT C.EMAIL,C.IS_ACCEPTED,C.MAIL_SEND,TO_DATE(C.PUB_DATE,'DD-MM-YY'),
C.IS_AUTOUPDATED,B.COMP_MID||'_'||A.USER_NAME,PASSWORDENCRYPT_DECRYPT(USER_PWD),
'CO'||LTRIM(TO_CHAR(TO_NUMBER(SUBSTR(A.GRP_COMP_ID,3,6))||TO_NUMBER(SUBSTR(A.COMP_AID,3,6)),'000000')),
A.USER_NAME,D.EMP_NAME,REMINDER1_DATE,REMINDER2_DATE FROM PI_GM_USERS A,PI_GM_COMP B,PI_GM_TRANS_FNF C,PI_GM_EMPMAST D 
...
FETCH curRec INTO v_Mailid,vIS_ACCEPTED,vMAIL_SEND,sgPUB_DATE,vIS_AUTOUPDATED,vUserName,vPassword,vCOMPDISPCODE1,PoornataID,V_EMP_NAME,vREMINDER1_DATE,vREMINDER2_DATE;
CLOSE curRec; OPEN curRec FOR SELECT EMP_NAME,CORRESPONDENCE_EMAIL1 FROM PI_GM_EMPMAST A,PI_GM_EMP_ATTRIBUTE_MAST B WHERE A.COMP_AID=B.COMP_AID AND A.EMP_AID=B.EMP_AID
AND B.ATTB_MID='SPOC' AND B.COMP_AID=pComp_Aid; FETCH curRec INTO V_EMP_NAMEcc,v_Mailidcc; CLOSE curRec;
vPUB_DATE:=TO_DATE(sgPUB_DATE);
LOOP IF TRIM(TO_CHAR(TO_DATE(vPUB_DATE),'MMDD'))IN('0126','0815','1002','1225','0501')
OR TO_CHAR(vPUB_DATE,'HH24:MI:SS')='16:00:00' THEN vPUB_DATE:=vPUB_DATE+1;ELSIF 
TRIM(TO_CHAR(TO_DATE(vPUB_DATE),'D'))IN('1','7') THEN vPUB_DATE:=vPUB_DATE+1;ELSE
vPUB_DATE:=vPUB_DATE;vCount:=vCount+1;IF TO_CHAR(SYSDATE,'DDMMYY')=
TO_CHAR(vPUB_DATE,'DDMMYY') THEN vNoOfDays:=vCount; END IF; END IF; EXIT WHEN 
TO_CHAR(SYSDATE,'DDMMYY')=TO_CHAR(vPUB_DATE,'DDMMYY');vPUB_DATE:=vPUB_DATE+1; END LOOP;
...
MAILBODY1 := MAILBODY1||''||''||''; MAILBODY1 := MAILBODY1||'';MAILBODY1 := MAILBODY1||'<P>Dear <b>' || V_EMP_NAME || ',</b> </p>';
...
MAILBODY1 := MAILBODY1||'<P><br>Best Regards,</P>';MAILBODY1 := MAILBODY1||'<P><b>Payroll Team - Osource</b></P>';MAILBODY1 := MAILBODY1||'</BODY></HTML>';
PRO_SEND_MAIL_NEW('payroll@osourceindia.com' ,v_Mailid,SUBJECT,MAILBODY1,NULL,NULL); UPDATE PI_GM_TRANS_FNF SET MAIL_SEND='Y' WHERE COMP_AID=pComp_Aid AND EMP_AID=pEmp_Aid;
ELSE IF NVL(vIS_ACCEPTED,'N')='N' AND NVL(vIS_AUTOUPDATED,'N')='N' THEN
...
MAILBODY1 := MAILBODY1||'<p> Note: This is a system generated email from an unmonitored mailbox. </P>';
MAILBODY1 := MAILBODY1||'<P><br>Best Regards,</P>';MAILBODY1 := MAILBODY1||'<P><b>Payroll Team - Osource</b></P>';MAILBODY1 := MAILBODY1||'</BODY></HTML>';
PRO_SEND_MAIL_NEW('payroll@osourceindia.com' ,v_Mailid,SUBJECT,MAILBODY1,NULL,NULL);PRO_DATE_REMINDER(pComp_Aid,pEmp_Aid,vNoOfDays);
